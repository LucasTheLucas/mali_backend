...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const contas = await db.sequelize.query(
            `SELECT id, email, senha, trocarsenha, funcao FROM usuarios WHERE email = :emailSolicitado AND senha = :senhaHash AND ativo = 'T'`,
            {
...
app.get("/dashboard", async (req, res) => {
    ...
    try {
        ...
        const totalLixeiras = await db.sequelize.query(`
            SELECT COUNT(*) total
            FROM lixeiras
        `);

        const totalUsuarios = await db.sequelize.query(`
            SELECT COUNT(*) total
            FROM usuarios
        `);

        const lixeirasCriticas = await db.sequelize.query(`
            SELECT COUNT(*) total
            FROM (
                SELECT
                    codlixeira,
        ...
        const mediaOcupacao = await db.sequelize.query(`
            SELECT
...
app.get("/dashboard/evolucao", async (req, res) => {
    ...
    try {
        ...
        const dados = await db.sequelize.query(`
            SELECT
                DATE(data) dia,
                ROUND(AVG(porcentagem),0) media
            FROM lixeiramovimentos
...
app.post("/cadastrarlixeira", async (req, res) => {
    ...
    try {
        ...
        console.error("Erro ao cadastrar lixeira:", erro);
